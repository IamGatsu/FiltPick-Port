...
public class BlockedItemsHud {
    ...
    private static class RenderContext {
        ...
        RenderContext(GuiGraphicsExtractor guiGraphics, Minecraft mc) {
        ...
        // TODO (26.2 port, please verify): renderFakeItem doesn't exist on GuiGraphicsExtractor anymore
        //  (confirmed by your compiler output). submitFakeItem is my best guess, following the new
        //  "submit"-prefixed naming used for item render states (GuiItemRenderState) in 26.x. If this also
        //  fails, please check autocomplete on "guiGraphics." in your IDE for the real name.
       void renderItem(Item item, int slotY) {
    guiGraphics.fakeItem(new ItemStack(item), baseX + ITEM_OFFSET, slotY + ITEM_OFFSET);
}
